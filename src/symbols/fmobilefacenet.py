...
def DResidual_v1(data, num_out=1, kernel=(3, 3), stride=(2, 2), pad=(1, 1), num_group=1, name=None, suffix=''):
...
def Residual(data, num_block=1, num_out=1, kernel=(3, 3), stride=(1, 1), pad=(1, 1), num_group=1, name=None, suffix=''):
    ...
    for i in range(num_block):
    	shortcut=identity
    	conv=DResidual_v1(data=identity, num_out=num_out, kernel=kernel, stride=stride, pad=pad, num_group=num_group, name='%s%s_block' %(name, suffix), suffix='%d'%i)
...
def get_symbol(num_classes, num_layers, **kwargs):
    ...
    """
    conv_1 = Conv(data, num_filter=filter_list[0], kernel=(3, 3), pad=(1, 1), stride=(2, 2), name="conv_1")
    if units[0] == 0:
        conv_2 = Conv(conv_1, num_group=filter_list[1], num_filter=filter_list[1], kernel=(3, 3), pad=(1, 1), stride=(1, 1), name="conv_2_dw")
    else:
