...
python -u train_softmax.py --lr 0.01 --fc7-lr-mult 1 --fc7-wd-mult 10 --lr-steps "$LRSTEPS" --data-dir $DATA_DIR --network "$NETWORK" --width-mult 1 --loss-type 4 --margin-m 0.5 --prefix "$PREFIX" --per-batch-size 64 --verbose 20000 --ckpt 2 --max-steps  800001 --pretrained "$PREFIX,10" --version-output "GDC" # > "$LOGFILE" 2>&1 &
